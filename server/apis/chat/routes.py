...
from apis.nexabot.guardrails import adjust_prompt_after_error
...
async def talk_session(session_id: str, request: Request):
    try:
        ...
        prompt = adjust_prompt_after_error(session_data.messages[-5:] + [HumanMessage(content=user_query)])

        can_perform_result = can_perform(prompt["prompt"], agent_name)
        if can_perform_result:

            approval_result = check_approval(prompt["prompt"], can_perform_result[0], session_data.messages, agent_name)
            ...
            if approval_result['status'] == 'Approved':
                ...
                print("User Query: ", user_query, "Action::Disapproved")

                # Search Approval
                
                doc_approval = can_answer_from_docs(prompt["prompt"], agent_name)
                print("Docs approval: ", doc_approval)
                if doc_approval["status"] == "Approved":
                    print("User Query: ", prompt, "Documents::Approved")
                    process_stream = session_manager.talk(session, nexabot, user_query)
                    User.decrease_available_limit(user_id)
                    return StreamingResponse(process_stream, media_type="text/plain")
                else:
                    print("User Query: ", prompt, "Documents::Disapproved")
